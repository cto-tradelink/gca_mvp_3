       ...
       console.log($("#int_sp"))
        ...
        init_year_data:function(){
            ...
            try{
                ...
                if( parseInt(this.startup.history[k].year) < min_year )
                    min_year =  parseInt(this.startup.history[k].year)
            ...
            try
            {
            for(var k =0; k <= this.startup.revenue.length ; k++){
                if( parseInt(this.startup.revenue[k].year) < min_year )
                    min_year =  parseInt(this.startup.revenue[k].year)
                if( parseInt(this.startup.revenue[k].year) > current_year )
                    current_year =  parseInt(this.startup.revenue[k].year)
                    }
            }
            catch(e){
                console.log(e)
            ...
            for(var k =0; k <= this.startup.trade.length ; k++){
                alert("for문 4")
                try{
                if( parseInt(this.startup.trade[k].year) < min_year_trade )
                    min_year_trade =  parseInt(this.startup.trade[k].year)
                if( parseInt(this.startup.trade[k].year) > current_year_trade )
                    current_year_trade =  parseInt(this.startup.trade[k].year)
                }
                catch(e){
                    console.log(e)
                }
        ...
        modify_trade:function(e){
            if($(e.path[0]).text()=="수정"){
                ...
                for(var k=0; k <= this.startup.trade.length ; k++ ){
                    if(this.startup.trade[k].id == parseInt( $(e.path[0]).attr("data-id") )){
                        this.startup.trade[k].num=r
                    } 
                }
            }
        },    
        add_tag:function(e){
            ...
            if(this.startup.tag.indexOf($(e.path[0]).text().replace("#",""))==-1){
                this.startup.tag.push($(e.path[0]).text().replace("#",""))
                $(e.path[0]).addClass("bold")
            } 
            else{
                var index = this.startup.tag.indexOf($(e.path[0]).text().replace("#",""))
                this.startup.tag.splice(index,1)

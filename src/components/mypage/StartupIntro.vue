    ...
    <div id="startup_intro_con" class="startup_detail panel_1 hidden">
        <div id="startup_info" class="">
            ...
            <div class="info_con">
                ...
                <div class="inner_wr" style="">
                    <ul class="inner_ul">
                        <li class="inner_li"  v-for="h in sta.history">
                            <span class="h_ttl" >{{h.year}}.{{h.month}}</span>
                            <span class="h_content">{{h.content}}</span>
                        </li>
                    </ul>
                </div> 
            </div>
        ...
        <div class="info_con">
            ...
            <ul>
                ...
                <span class="h_content">{{h.content}}</span></li>
            </ul>
            <div id="add_history" v-on:click="add_history"><img src="/static/img/pluse4.png" style="float:left; margin-top:12px;margin-left:252px;cursor:pointer"> 추가하기</div>
        </div>
...
export default {
    ...
    methods:{
        ...
        add_history:function(){
            ...
            var content = $("#history_con").val()
            this.sta.history.push({
                year:year,month:month, content:content,                
            })
            this.date=""
            $("#history_con").val("")
        },
        add_filter:function(string){
          try{
            if(select_tag.indexOf(string) != -1) {
               select_tag.slice(select_tag.indexOf(string),1)
           }else{
               select_tag.push(string)
           }
           //this.startup.select_tag=select_tag.slice(0)
          }
        catch(e){
            console.log(e)
        }
        ...
        filter_popup_toggle:function(){
            if($("#filter_seg").hasClass("hidden")){
                $("#filter_seg").removeClass("hidden")
  ...
  var values = ["제한없음"]
            ...
            .then((result) => {  
                ...
                $("#startup_info").removeClass("hidden")
                $("#startup_modi").addClass("hidden")
            })
        ...
        // 주소-좌표 변환 객체를 생성합니다
        var geocoder = new daum.maps.services.Geocoder();
        var address_start = vue_obj.sta.location
        // 주소로 좌표를 검색합니다
